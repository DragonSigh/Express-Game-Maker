...
namespace WeifenLuo.WinFormsUI.Docking
{
    internal class VS2005DockPaneStrip : DockPaneStripBase
    {
        ...
        // CHANGE
        private const int _DocumentTabMinWidth = 00;
        ...
        private static string ToolTipClose
        {
            get
            {
                if (m_toolTipClose == null)
                    m_toolTipClose = EGMGame.Properties.Resources.DockPaneStrip_ToolTipClose;
        ...
        private static string ToolTipSelect
        {
            get
            {
                if (m_toolTipSelect == null)
                    m_toolTipSelect = EGMGame.Properties.Resources.DockPaneStrip_ToolTipWindowList;
        ...
        private static int DocumentTabMinWidth
        {
            get { return _DocumentTabMinWidth; }
        }
        ...
        private bool CalculateDocumentTab(Rectangle rectTabStrip, ref int x, int index)
        {
            ...
            if (x + width < rectTabStrip.Right || index == StartDisplayingTab)
            {
                tab.TabX = x;
                // CHANGE 
                //if (width < DocumentTabMinWidth)
                //    width = DocumentTabMinWidth;
                tab.TabWidth = width;
        ...
        private int GetMaxTabWidth_Document(int index)
        {
            ...
            if (DockPane.DockPanel.ShowDocumentIcon)
                ...
                return sizeText.Width + DocumentIconGapLeft + DocumentTextGapRight + m_imageButtonClose.Width;
        }
        ...
        private GraphicsPath GetTabOutline_Document(Tab tab, bool rtlTransform, bool toScreen, bool full)
        {
            ...
            // CHANGE 
            return DrawHelper.RoundedRectangle(rect, 3,
                                        DrawHelper.RectangleCorners.TopLeft |
                                        DrawHelper.RectangleCorners.TopRight);
        ...
        private void DrawTab_Document(Graphics g, TabVS2005 tab, Rectangle rect)
        {
            ...
            if (DockPane.ActiveContent == tab.Content)
            {
                ...
                {
                    var rectCloseButton = GetCloseButtonRect(rect);
                    if (IsMouseOverRect(rectCloseButton))
                    {
                        g.DrawImage(Resources.DockCloseHover, rectCloseButton);
                    }
                    else
                        g.DrawImage(Resources.DockClose, rectCloseButton);
                }
            }
            else
            {
                // CHANGE
                if (IsMouseOverRect(rectTab))
                {
                    Color startColor =
                        DockPane.DockPanel.Skin.DockPaneStripSkin.DocumentGradient.InactiveTabGradient.StartColor;
                    startColor = Color.FromArgb(100, startColor);
                    Color endColor =
                        DockPane.DockPanel.Skin.DockPaneStripSkin.DocumentGradient.InactiveTabGradient.EndColor;
                    endColor = Color.FromArgb(100, endColor);

                    LinearGradientMode gradientMode =
                        DockPane.DockPanel.Skin.DockPaneStripSkin.DocumentGradient.InactiveTabGradient.
                            LinearGradientMode;
                    g.FillPath(new LinearGradientBrush(rectBack, startColor, endColor, gradientMode), path);
        ...
        {
            const int gap = 4;
            var dimension = 16; //rectTab.Height - gap; 
            return new Rectangle(rectTab.X + rectTab.Width - dimension - gap / 2, rectTab.Y + gap / 2, dimension, dimension);
        }

        // CHANGE
        private bool IsMouseOverRect(Rectangle closeRect)
        {
            var test = new Rectangle(_mousex,_mousey,1,1);
            return closeRect.IntersectsWith(test);
        }
        ...
        {
            base.OnMouseMove(e);
            if (!DockPane.DockPanel.ShowCloseButtonOnEachTab || e.Button != MouseButtons.None)
                return;
            Refresh();
            _mousex = e.X;
            _mousey = e.Y;
        ...
        private void SetInertButtons()
        {
            ...
            else
            {
                ...
                // CHANGE
                m_closeButtonVisible = false;//DockPane.ActiveContent == null ? true : DockPane.ActiveContent.DockHandler.CloseButtonVisible;
