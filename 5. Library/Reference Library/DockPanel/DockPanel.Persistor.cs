    ...
    partial class DockPanel
    {
        private static class Persistor
        {
            ...
            public static void SaveAsXml(DockPanel dockPanel, Stream stream, Encoding encoding, bool upstream)
            {
                ...
                xmlOut.WriteComment(EGMGame.Properties.Resources.DockPanel_Persistor_XmlFileComment1);
                xmlOut.WriteComment(EGMGame.Properties.Resources.DockPanel_Persistor_XmlFileComment2);
            ...
            private static ContentStruct[] LoadContents(XmlTextReader xmlIn)
            {
                ...
                for (int i = 0; i < countOfContents; i++)
                {
                    ...
                    if (xmlIn.Name != "Content" || id != i)
                        throw new ArgumentException(EGMGame.Properties.Resources.DockPanel_LoadFromXml_InvalidXmlFormat);
            ...
            public static void LoadFromXml(DockPanel dockPanel, Stream stream, DeserializeDockContent deserializeContent, bool closeStream)
            {
                ...
                if (dockPanel.Contents.Count != 0)
                    throw new InvalidOperationException(EGMGame.Properties.Resources.DockPanel_LoadFromXml_AlreadyInitialized);
                ...
                if (!IsFormatVersionValid(formatVersion))
                    throw new ArgumentException(EGMGame.Properties.Resources.DockPanel_LoadFromXml_InvalidFormatVersion);
                ...
                for (int i = 0; i < dockWindows.Length; i++)
                {
                    for (int j = 0; j < dockWindows[i].NestedPanes.Length; j++)
                    {
                        ...
                        if (indexPane < dockPanel.Panes.Count)
                        {
                ...
                for (int i = 0; i < panes.Length; i++)
                    if (panes[i].IndexActiveContent > -1)
