    ...
    public partial class DockPane : UserControl, IDockDragSource
    {
        ...
        private void InternalConstruct(IDockContent content, DockState dockState, bool flagBounds, Rectangle floatWindowBounds, DockPane prevPane, DockAlignment alignment, double proportion, bool show)
        {
            if (dockState == DockState.Hidden || dockState == DockState.Unknown)
                throw new ArgumentException(EGMGame.Properties.Resources.DockPane_SetDockState_InvalidState);

            if (content == null)
                throw new ArgumentNullException(EGMGame.Properties.Resources.DockPane_Constructor_NullContent);

            if (content.DockHandler.DockPanel == null)
                throw new ArgumentException(EGMGame.Properties.Resources.DockPane_Constructor_NullDockPanel);
            ...
            set
            {
                ...
                if (value != null)
                {
                    if (!DisplayingContents.Contains(value))
                        value = null;//  throw (new InvalidOperationException(EGMGame.Properties.Resources.DockPane_ActiveContent_InvalidValue));
                }
                else
                {
                    if (DisplayingContents.Count != 0)
                        throw (new InvalidOperationException(EGMGame.Properties.Resources.DockPane_ActiveContent_InvalidValue));
                }
        ...
        public void SetContentIndex(IDockContent content, int index)
        {
            ...
            if (oldIndex == -1)
                throw (new ArgumentException(EGMGame.Properties.Resources.DockPane_SetContentIndex_InvalidContent));

            if (index < 0 || index > Contents.Count - 1)
                if (index != -1)
                    throw (new ArgumentOutOfRangeException(EGMGame.Properties.Resources.DockPane_SetContentIndex_InvalidIndex));
        ...
        public DockPane SetDockState(DockState value)
        {
            if (value == DockState.Unknown || value == DockState.Hidden)
                throw new InvalidOperationException(EGMGame.Properties.Resources.DockPane_SetDockState_InvalidState);
        ...
        {
            if (container == null)
                throw new InvalidOperationException(EGMGame.Properties.Resources.DockPane_DockTo_NullContainer);
        ...
        private void InternalAddToDockList(INestedPanesContainer container, DockPane prevPane, DockAlignment alignment, double proportion)
        {
            if ((container.DockState == DockState.Float) != IsFloat)
                throw new InvalidOperationException(EGMGame.Properties.Resources.DockPane_DockTo_InvalidContainer);

            int count = container.NestedPanes.Count;
            if (container.NestedPanes.Contains(this))
                count--;
            if (prevPane == null && count > 0)
                throw new InvalidOperationException(EGMGame.Properties.Resources.DockPane_DockTo_NullPrevPane);

            if (prevPane != null && !container.NestedPanes.Contains(prevPane))
                throw new InvalidOperationException(EGMGame.Properties.Resources.DockPane_DockTo_NoPrevPane);

            if (prevPane == this)
                throw new InvalidOperationException(EGMGame.Properties.Resources.DockPane_DockTo_SelfPrevPane);
