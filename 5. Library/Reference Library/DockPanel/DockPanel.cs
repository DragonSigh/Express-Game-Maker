...
namespace WeifenLuo.WinFormsUI.Docking
{
	...
	public partial class DockPanel : Panel
	{
        ...
        // CHANGE (Prop)
        private float m_floatingWindowInactiveOpacity = .3f;
        [DefaultValue(true)]
        [LocalizedCategory("Category_Appearance")]
        public float FloatingWindowInactiveOpacity
        {
            get { return m_floatingWindowInactiveOpacity; }
            set
            {
                if (m_floatingWindowInactiveOpacity == value)
                    return;
                m_floatingWindowInactiveOpacity = value;
            }
        }
		...
		public void SetPaneIndex(DockPane pane, int index)
		{
			...
			if (oldIndex == -1)
				throw(new ArgumentException(EGMGame.Properties.Resources.DockPanel_SetPaneIndex_InvalidPane));

			if (index < 0 || index > Panes.Count - 1)
				if (index != -1)
					throw(new ArgumentOutOfRangeException(EGMGame.Properties.Resources.DockPanel_SetPaneIndex_InvalidIndex));
	    ...
	    internal Form ParentForm
		{
			...
			{	
				if (!IsParentFormValid())
					throw new InvalidOperationException(EGMGame.Properties.Resources.DockPanel_ParentForm_Invalid);
