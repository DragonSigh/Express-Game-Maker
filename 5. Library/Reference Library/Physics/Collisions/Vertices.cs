...
namespace FarseerPhysics.Common
{
    ...
    public class Vertices : List<Vector2>
    {
        ...
        public void Rotate(float value)
        {
            ...
            for (int i = 0; i < Count; i++)
            {
                Vector2 result, B = new Vector2(32, 32); float C = value;
                result.X = (this[i].X - B.X) * (float)Math.Cos(C) - (this[i].Y - B.Y) * (float)Math.Sin(C) + B.X;
                result.Y = (this[i].Y - B.Y) * (float)Math.Cos(C) + (this[i].X - B.X) * (float)Math.Sin(C) + B.Y;
                this[i] = result;
        ...
        /// <returns></returns>
        public static bool VerticesAreCollinear(Vector2 p1, Vector2 p2, Vector2 p3)
        {
