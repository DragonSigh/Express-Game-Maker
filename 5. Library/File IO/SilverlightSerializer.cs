    ...
    public static class SilverlightSerializer
    {
        ...
        private static IEnumerable<PropertyInfo> GetPropertyInfo(Type itm)
        {
            lock (PropertyLists)
            {
                ...
                {
                    ret = itm.GetProperties(BindingFlags.Instance | BindingFlags.Public | BindingFlags.FlattenHierarchy).Where(p => p.GetCustomAttributes(typeof(DoNotSerialize), false).Count() == 0 && !(p.GetIndexParameters().Count() > 0) && (p.GetSetMethod() != null)).ToArray();
        ...
        private static IEnumerable<FieldInfo> GetFieldInfo(Type itm)
        {
            lock (FieldLists)
            {
                ...
                FieldLists[itm] = itm.GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.FlattenHierarchy | BindingFlags.SetField).Where(p => p.GetCustomAttributes(typeof(DoNotSerialize), false).Count() == 0).ToArray();
        ...
        public static object Deserialize(byte[] bytes)
        {
            ...
            try
            {
                ...
                {
                    var tp = Type.GetType(rw.ReadString());
                    _knownTypes.Add(tp);
                }
                count = rw.ReadInt32();
                for (var i = 0; i < count; i++)
        ...
        /// </remarks>
        private static object DeserializeObject(BinaryReader reader, Type itemType = null)
        {
        ...
        private static void SerializeObject(object item, BinaryWriter writer, Type propertyType = null)
        {
            ...
            if (_seenObjects.ContainsKey(item) && !itemType.IsValueType)
            {
                writer.Write('S');
        ...
        private static void WriteProperties(Type itemType, object item, BinaryWriter writer)
        {
            ...
            foreach (var property in properties)
            {
                ...
                if (item is List<TileData> && property.Name == "DisplayRect")
