...
{
    public partial class SpriteGridDirControl : UserControl
    {
        #region Fields
        public AnimationSprite SelectedSprite
        {
            ...
            set
            {
                ...
                if (camera != null)
                {
                    Texture2D tex = GetTexture(selSprite);
        ...
        internal AnimationSprite cloneSprite;

        internal Vector2 Grid = new Vector2(1, 1);

        internal MaterialData Data;

        internal Vector4 selection = Vector4.Zero;
        ...
        private void graphicsControl_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (Data != null)
            {
                ...
                Texture2D tex = Loader.Texture2D(contentManager, Data.ID);
                if (tex != null)
                {
                    ...
                    {
                        IsMouseDown = true;
                        float X = (float)Math.Floor((double)(point.X / (tex.Width / Grid.X))) * (tex.Width / Grid.X);
                        float Y = (float)Math.Floor((double)(point.Y / (tex.Height / Grid.Y))) * (tex.Height / Grid.Y);
                        float sx = (float)Math.Floor((double)(point.X / (tex.Width / Grid.X)));
                        float sy = (float)Math.Floor((double)(point.Y / (tex.Height / Grid.Y)));
                        // Set sprite bounds
                        selection.X = sx;
                        selection.Y = sy;
                        selection.Z = 1;
        ...
        private void graphicsControl_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            if (selSprite != null)
            {
                System.Drawing.PointF point = camera.GetTransformedPoint(e.Location);
                // Divide Point by width and height
                Texture2D tex = GetTexture(SelectedSprite);
                System.Drawing.RectangleF rect = new System.Drawing.RectangleF(0, 0, tex.Width, tex.Height);
                if (rect.Contains(point))
                {
                    float X = (float)Math.Floor((double)(point.X / selSprite.DisplayRect.Width)) * selSprite.DisplayRect.Width;
                    float Y = (float)Math.Floor((double)(point.Y / selSprite.DisplayRect.Height)) * selSprite.DisplayRect.Height;
                    // Set sprite bounds
                    Rectangle r = selSprite.DisplayRect;
                    r.X = (int)X;
                    r.Y = (int)Y;
                    selSprite.DisplayRect = r;
                    if (selSprite.DisplayRect != cloneSprite.DisplayRect)
                    {
                        AnimationSprite sprite = Global.Duplicate<AnimationSprite>(selSprite);
                        selSprite.DisplayRect = cloneSprite.DisplayRect;
                        MainForm.AnimationHistory[MainForm.animationEditor].Do(new IGameDataChangePropertyHist(selSprite));
                        selSprite.DisplayRect = sprite.DisplayRect;
                    }
                }
                else
                {
            ...
            ((EGMGame.Dialogs.FromSpriteSheetForDirDialog)this.Parent).btnOK_Click(null, EventArgs.Empty);
        }
        ...
        private void graphicsControl_MouseMove(object sender, MouseEventArgs e)
        {
            if (!ctrl && IsMouseDown)
            {
                ...
                //selection.Z = selection.X - sx;
                //selection.W = selection.Y - sy;
            }
        ...
        private void DrawSelected()
        {
            ...
            if (tex != null)
            {
                ...
                if (Columns)
                    selectionRect = new System.Drawing.RectangleF(tex.Width / Grid.X * selection.X, tex.Height / Grid.Y * selection.Y, tex.Width / Grid.X * Frames, tex.Height / Grid.Y);
                else
                    selectionRect = new System.Drawing.RectangleF(tex.Width / Grid.X * selection.X, tex.Height / Grid.Y * selection.Y, tex.Width / Grid.X, tex.Height / Grid.Y * Frames);

                //DrawRectangle(selectionRect, Color.Blue, 0);

                FillRectangle(selectionRect, Color.White, new Color(0, 0, 255, 100), 0);
            }
        }
        /// <summary>
        /// Draw Grid
        ...
        private void DrawGrid()
        {
            ...
            if (tex != null)
            {
                for (int x = 0; x <= tex.Width; x += tex.Width / (int)Grid.X)
                {
                    // Horizontal
                    if (x == 0)
                    {
                        DrawLine(new Vector2(1, 0), new Vector2(1, tex.Height), Color.Black, 0);
                    }
                    else
                    {
                        DrawLine(new Vector2(x, 0), new Vector2(x, tex.Height), Color.Black, 0);
                    }
                }
                // Vertical
                for (int y = 0; y <= tex.Height + 1; y += tex.Height / (int)Grid.Y)
                {
                    if (y == tex.Height)
                    {
                        DrawLine(new Vector2(0, y), new Vector2(tex.Width, y - 1), Color.Black, 0);
                    }
                    else
                    {
                        DrawLine(new Vector2(0, y), new Vector2(tex.Width, y), Color.Black, 0);
                    }
                }
            }
        }
