...
{
    public partial class TextComboBox : TreeViewComboBox
    {
        ...
        {
            InitializeComponent();
            Global.CbTexts.Add(this);
        }
        public TextComboBox(IContainer container)
        {
            container.Add(this);
        ...
        public override void RefreshList(bool memorize)
        {
            ...
            TreeNode node;
            int catIndex = 0;
            // Add All Texts To list
            if (noneable)
            {
                catIndex = 1;
                TextData an = new TextData();
                an.ID = -1;
                an.Name = "(none)";
            ...
            // Add Categories
            List<NodeCategory> categories = Global.Project.Categories[typeof(TextData).ToString()];
            ...
            foreach (TextData e in GameData.Texts["English"].Values)
            {
        ...
        public TextData Data()
        {
            if (this.SelectedNode != null && this.SelectedNode.Tag != null)
                return (TextData)this.SelectedNode.Tag;
            else
            {
                TextData dd = new TextData();
                dd.ID = -10;
                dd.Name = "{No Text}";
                return dd;
            }
