...
{
    public partial class SkillsComboBox : TreeViewComboBox
    {
        ...
        bool useSkillType = false;
        /// <summary>
        /// Initialize
        /// </summary>
        public SkillsComboBox()
        {
            InitializeComponent();
            //this.DropDownStyle = ComboBoxStyle.DropDownList;

            Global.CbSkills.Add(this);
        }
        public SkillsComboBox(IContainer container)
        {
            container.Add(this);
        ...
        /// Refreshes List
        /// </summary>
        public void RefreshList(bool memorize, SkillType type)
        {
            useSkillType = true;
            // Memorize
            IGameData i = null;
            ...
            // Add Categories
            List<NodeCategory> categories = Global.Project.Categories[typeof(SkillData).ToString()];
            ...
            foreach (SkillData e in GameData.Skills.Values)
            {
                if (e.SkillType == type || type == SkillType.Both)
                {
        ...
        {
            if (useSkillType)
                RefreshList(memorize, skillType);
            else
            {
                useSkillType = false;
                // Memorize
        ...
        public SkillData Data()
        {
            if (this.SelectedNode != null && this.SelectedNode.Tag != null)
                return (SkillData)this.SelectedNode.Tag;
            else
            {
                SkillData dd = new SkillData();
                dd.ID = -10;
                dd.Name = "{No Skill}";
                return dd;
            }
