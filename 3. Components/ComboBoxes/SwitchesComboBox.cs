...
{
    public partial class SwitchesComboBox : TreeViewComboBox
    {
        IEvent selEvent;
        /// <summary>
        /// Initialize
        /// </summary>
        public SwitchesComboBox()
        {
            InitializeComponent();
            //this.DropDownStyle = ComboBoxStyle.DropDownList;

            Global.CbSwitches.Add(this);
        }
        public SwitchesComboBox(IContainer container)
        {
            container.Add(this);
        ...
        public override void RefreshList(bool memorize)
        {
            ...
            else
            {
                ...
                TreeNode node;
                // Add Categories
                List<NodeCategory> categories = Global.Project.Categories[typeof(SwitchData).ToString()];
                ...
                foreach (SwitchData e in GameData.Switches.Values)
                {
        ...
        internal void RefreshList(IEvent ev, bool memorize)
        {
            selEvent = ev;
            // Memorize
            IGameData i = null;
            ...
            // Add All Items To list
            foreach (SwitchData e in ev.Switches.Values)
            {
                if (!(string.IsNullOrEmpty(txtSearch.Text) || e.Name.ToLower().Contains(txtSearch.Text.ToLower())))
                    continue;
                node = new TreeNode(e.Name);
        ...
        public SwitchData Data()
        {
            if (this.SelectedNode != null && this.SelectedNode.Tag != null)
                return (SwitchData)this.SelectedNode.Tag;
            else
            {
                SwitchData dd = new SwitchData();
                dd.ID = -10;
                dd.Name = "{No Switch}";
                return dd;
            }
