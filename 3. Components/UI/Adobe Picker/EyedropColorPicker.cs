...
namespace EGMGame
{
	class EyedropColorPicker : Control
	{
		...
		public EyedropColorPicker()
		{
			...
			//m_icon = new Bitmap(typeof(EyedropColorPicker), Properties.Resources.eyedropper);
            m_icon = new Bitmap(Properties.Resources.eyedropper);
		...
		RectangleF ImageRect
		{
			get
			{
				return EGMColorUtil.Rect(ClientRectangle);
			}
		}
		...
		void GetSnapshot()
		{
			...
			using (Graphics dc = Graphics.FromImage(m_snapshot))
			{
				...
				PointF center = EGMColorUtil.Center(new RectangleF(0, 0, m_snapshot.Size.Width, m_snapshot.Size.Height));
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			if (m_snapshot != null)
			{
				...
				if (iscapturing)
				{
					PointF center = EGMColorUtil.Center(r);
					Rectangle centerrect = new Rectangle(EGMColorUtil.Point(center), new Size(0, 0));
			...
			EGMColorUtil.DrawFrame(e.Graphics, rr, 6, Color.CadetBlue);
		}
