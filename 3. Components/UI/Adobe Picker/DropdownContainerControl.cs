...
namespace EGMGame
{
	public class ColorPickerCombobox : DropdownContainerControl<Color>
	{
        ...
        #region Events
        // Event variables
        public delegate void ColorSelectedHandler(Color e);
        public event ColorSelectedHandler ColorSelectedEvent;
        #endregion
		...
		public ColorPickerCombobox()
		{
			...
			if (m_colorPicker == null)
            {
                ...
                m_colorPicker.colorInfo.ColorChanged += new EventHandler(colorInfo_ColorChanged);
				m_container.SetControl(m_colorPicker);
            }
            DropdownContainer = m_container;
		}

        void colorInfo_ColorChanged(object sender, EventArgs e)
        {
            SelectedItem = m_colorPicker.SelectedColor;
            if (ColorSelectedEvent != null)
                ColorSelectedEvent(SelectedItem);
        }

        [Browsable(true)]
        public bool AllowOpacity
        {
            get { return m_colorPicker.AllowOpacity; }
            set { m_colorPicker.AllowOpacity = value; }
        }
	...
	public class DropdownContainer<T> : Form
	{
		...
		public DropdownContainer(DropdownContainerControl<T> owner)
		{
			...
			m_cancelButton.Location = new Point(r.Right - m_cancelButton.Size.Width -3, r.Bottom - m_cancelButton.Height -3);
			m_cancelButton.Anchor = AnchorStyles.Bottom | AnchorStyles.Right;
			m_cancelButton.Click += new System.EventHandler(OnCancelClick);
			Controls.Add(m_cancelButton);
			
			m_acceptButton = new Button();
			m_acceptButton.Text = "OK";
			m_acceptButton.Size = new Size(60,m_buttonAreaHeight-2);
			m_acceptButton.Location = new Point(m_cancelButton.Left - m_acceptButton.Size.Width - 1 -3, r.Bottom - m_acceptButton.Height -3);
			m_acceptButton.Anchor = AnchorStyles.Bottom | AnchorStyles.Right;
			m_acceptButton.Click += new System.EventHandler(OnAcceptClick);
		...
		public virtual new Rectangle ClientRectangle
		{
			...
			{ 
				...
				r.Height -= m_frameMargin*2 + m_buttonAreaHeight;
				r.X	+= m_frameMargin;
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			r.Width--;
			r.Height--;
			EGMColorUtil.DrawFrame(e.Graphics, r, 6, Color.CadetBlue);

            //e.Graphics.DrawImage(Properties.Resources.cancel, CancelButtonRect);
            //e.Graphics.DrawImage(Properties.Resources.tick, AcceptButtonRect);
			//e.Graphics.DrawImage(.Image(PopupContainerImages.eIndexes.Close), CancelButtonRect);
