...
namespace EGMGame
{
	...
	public class ColorBox : System.Windows.Forms.UserControl
	{
		...
		public ColorBox()
		{
			...
			//	Initialize Colors
			m_hsl = new EGMColors.HSL();
			m_hsl.H = 1.0;
			m_hsl.S = 1.0;
			m_hsl.L = 1.0;
			m_rgb = EGMColors.HSL_to_RGB(m_hsl);
		...
		public Color RGB
		{
			...
			{
				m_rgb = value;
				m_hsl = EGMColors.RGB_to_HSL(m_rgb);
		...
		private void ClearMarker()
		{
			...
			EGMColors.HSL hsl_start = new EGMColors.HSL();
			EGMColors.HSL hsl_end = new EGMColors.HSL();
			...
			switch (m_eDrawStyle)
			{
				...
				case eDrawStyle.Hue :	
					...
					for ( int i = start_y; i <= end_y; i++ )		//	For each horizontal line:
					{
						...
						LinearGradientBrush br = new LinearGradientBrush(new Rectangle(start_x + 1,i + 2, end_x - start_x + 1, 1), EGMColors.HSL_to_RGB(hsl_start), EGMColors.HSL_to_RGB(hsl_end), 0, false); 
				...
				case eDrawStyle.Saturation :
					...
					for ( int i = start_x; i <= end_x; i++ )				//	For each vertical line:
					{
						...
						LinearGradientBrush br = new LinearGradientBrush(new Rectangle(i + 2,start_y + 1, 1, end_y - start_y + 2), EGMColors.HSL_to_RGB(hsl_start), EGMColors.HSL_to_RGB(hsl_end), 90, false); 
		...
		{																	//	  *  |  *
			...
			Pen pen;
			EGMColors.HSL _hsl = GetColor(x,y);	//	The selected color determines the color of the marker drawn over
			//	it (black or white)
		...
		private void DrawBorder()
		{
			...
			//	To make the control look like EGM Photoshop's the border around the control will be a gray line
		...
		private void Draw_Style_Hue()
		{
			...
			for ( int i = 0; i < this.Height - 4; i++ )				//	For each horizontal line in the control:
			{
				...
				LinearGradientBrush br = new LinearGradientBrush(new Rectangle(2,2, this.Width - 4, 1), EGMColors.HSL_to_RGB(hsl_start), EGMColors.HSL_to_RGB(hsl_end), 0, false); 
		...
		private void Draw_Style_Saturation()
		{
			...
			for ( int i = 0; i < this.Width - 4; i++ )		//	For each vertical line in the control:
			{
				...
				LinearGradientBrush br = new LinearGradientBrush(new Rectangle(2,2, 1, this.Height - 4), EGMColors.HSL_to_RGB(hsl_start), EGMColors.HSL_to_RGB(hsl_end), 90, false); 
		...
		/// <param name="y"></param>
		/// <returns></returns>
		private EGMColors.HSL GetColor(int x, int y)
		{

			EGMColors.HSL _hsl = new EGMColors.HSL();
			...
			switch (m_eDrawStyle)
			{
				...
				case eDrawStyle.Brightness :
					...
					break;
				case eDrawStyle.Red :
					_hsl = EGMColors.RGB_to_HSL(Color.FromArgb(m_rgb.R, Round(255 * (1.0 - (double)y/(this.Height - 4))), Round(255 * (double)x/(this.Width - 4))));
					break;
				case eDrawStyle.Green :
					_hsl = EGMColors.RGB_to_HSL(Color.FromArgb(Round(255 * (1.0 - (double)y/(this.Height - 4))), m_rgb.G, Round(255 * (double)x/(this.Width - 4))));
					break;
				case eDrawStyle.Blue :
					_hsl = EGMColors.RGB_to_HSL(Color.FromArgb(Round(255 * (double)x/(this.Width - 4)), Round(255 * (1.0 - (double)y/(this.Height - 4))), m_rgb.B));
					break;
			}
