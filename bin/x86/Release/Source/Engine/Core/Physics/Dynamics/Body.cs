...
namespace FarseerPhysics.Dynamics
{
    ...
    public class Body : IDisposable
    {
        ...
        public Body(World world, object userData)
        {
            ...
            ControllerFilter.RestoreController(ControllerType.GravityController);
        }
        ...
        #region EXPRESS GAME MAKER PATCH
        /// <summary>
        /// Returns or sets the minimum velocity. If the body's velocity is below this Value, it can
        /// be deactivated
        /// </summary>
        public float MinimumVelocity = ConvertUnits.ToSimUnits(20); // 40
        ...
        internal void ResetTorque()
        {
            AngularVelocity = 0;
            AngularVelocityInternal = 0;
        }

        internal void ResetBaseDynamics()
        {
        ...
        internal void ClearForce()
        {
            LinearVelocity = Vector2.Zero;
        }

        internal void ClearImpulse()
        {
            LinearVelocityInternal = Vector2.Zero;
        }

        internal void ClearTorque()
        {
            AngularVelocity = 0;
            AngularVelocityInternal = 0;
        }
        ...
        internal void ResetIgnore()
        {
            for (int i = 0; i < FixtureList.Count; i++)
            {
                FixtureList[i].ResetIgnore();
            }
        }
        #endregion

        DistanceInput _distanceInput = new DistanceInput();
        SimplexCache cache;
        internal float GetDistance(Body body)
        {
            float min = float.MaxValue;
            DistanceOutput distanceOutput;
            _distanceInput.TransformA = this.Xf;
            _distanceInput.TransformB = body.Xf;

            foreach (Fixture fixture in FixtureList)
            {
                for (int i = 0; i < fixture.Shape.ChildCount; i++)
                {
                    _distanceInput.ProxyA.Set(fixture.Shape, i);

                    foreach (Fixture fixture2 in body.FixtureList)
                    {
                        for (int j = 0; j < fixture2.Shape.ChildCount; j++)
                        {
                            _distanceInput.ProxyB.Set(fixture2.Shape, j);
                            Distance.ComputeDistance(out distanceOutput, out cache, _distanceInput);
                            if (distanceOutput.Distance < min)
                                min = distanceOutput.Distance;
                        }
                    }
                }
            }
        ...
        {
            return RayCast(start, end, 1f);
        }

        public bool RayCast(Vector2 start, Vector2 end, float fraction)
        {
            ...
            input.Point2 = end;
            bool touch = false;
            if (DebugViewXNA.RecordRay) DebugViewXNA.RayCastInputs.Add(input);
            DebugViewXNA.RecordRay = false;
            //DebugViewXNA.RayCastInputs.Add(input);
            if (DebugViewXNA.RayCastInputs.Count > 50) DebugViewXNA.RayCastInputs.RemoveAt(0);
            foreach (Fixture fixture in FixtureList)
            {
                for (int i = 0; i < fixture.Shape.ChildCount; i++)
                {
                    touch = fixture.Shape.RayCast(out output, ref input, ref Xf, i);
                    if (touch) break;
                }
                if (touch) break;
            }
        ...
        public float Height
        {
            get
            {
                ...
                foreach (Fixture fixture in FixtureList)
                {
                    for (int i = 0; i < fixture.Shape.ChildCount; i++)
                    {
                        fixture.Shape.ComputeAABB(out aabb, ref Xf, i);

                        if (aabb.LowerBound.X < min.X)
                            min.X = aabb.LowerBound.X;
                        if (aabb.LowerBound.Y < min.Y)
                            min.Y = aabb.LowerBound.Y;
                        if (aabb.UpperBound.X > max.X)
                            max.X = aabb.UpperBound.X;
                        if (aabb.UpperBound.Y > max.Y)
                            max.Y = aabb.UpperBound.Y;
                    }
                }
        ...
        internal bool Collide(Body body)
        {
            Manifold m = new Manifold();

            foreach (Fixture fixture1 in FixtureList)
            {

                foreach (Fixture fixture2 in body.FixtureList)
                {
                    Collision.Collision.CollidePolygons(ref m,
                                                        (PolygonShape)fixture1.Shape, ref Xf,
                                                        (PolygonShape)fixture2.Shape, ref body.Xf);
                    if (m.PointCount > 0)
                        return true;
                }
            }
